...
void render_egl_image_dma_buf(int width, int height){
	...
	gBoTest = gbm_bo_create(gDevice, width, height, GBM_BO_FORMAT_XRGB8888, GBM_BO_USE_RENDERING);
	if(!gBoTest){
	...
	pRawData = gbm_bo_map(gBoTest, 0, 0, width, height, GBM_BO_TRANSFER_READ_WRITE, &mapStride, &mapData);
	if(pRawData){
		pData = pRawData;
		for(i=0; i<width; i++){
			for(j=0; j<height; j++){
	...
	eglImage = (*pfnCreateImageKHR)(eglDisplay, EGL_NO_CONTEXT, EGL_LINUX_DMA_BUF_EXT, NULL, eglAttribs);
	if(!eglImage){
	...
	(*pfnDestroyImageKHR)(eglDisplay, eglImage);
	gbm_bo_destroy(gBoTest);
...
void swap_buffers(void){
	unsigned int handle=0, pitch=0, fb=0;
...
int main(int argc, char *argv[]){
	...
	render_egl_image_dma_buf(kms.mode.hdisplay, kms.mode.vdisplay);
